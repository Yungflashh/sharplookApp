...
const VendorSidebar: React.FC<VendorSidebarProps> = ({
  ...
  return <Modal visible={visible} transparent animationType="none" onRequestClose={onClose} statusBarTranslucent>
      <View className="flex-1">
        ...
        {}
        <Animated.View className="absolute left-0 top-0 bottom-0 bg-white" style={{
        width: DRAWER_WIDTH,
        transform: [{
        ...
        ...Platform.select({
          ...
          <SafeAreaView className="flex-1" edges={['top']}>
            {}
            <LinearGradient colors={['#ec4899', '#f472b6']} start={{
            x: 0,
            y: 0
          }} end={{
            x: 1,
            ...
            <ScrollView className="flex-1 pt-4" showsVerticalScrollIndicator={false}>
              {menuSections.map((section, sectionIndex) => <View key={sectionIndex} className="mb-6">
                  ...
                  <View>
                    {section.items.map((item, itemIndex) => <TouchableOpacity key={item.id} onPress={item.onPress} className="flex-row items-center px-6 py-3.5 active:bg-gray-50" activeOpacity={0.7}>
                        ...
                        {item.badge && <View className="bg-pink-500 px-2 py-0.5 rounded-full min-w-[24px] items-center">
