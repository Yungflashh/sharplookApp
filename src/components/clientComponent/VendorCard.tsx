...
const VendorCard: React.FC<VendorCardProps> = ({
  ...
  const renderStars = (rating: number) => {
    return <View className="flex-row items-center">
        {[1, 2, 3, 4, 5].map(star => <Ionicons key={star} name={star <= rating ? 'star' : 'star-outline'} size={17} color={star <= rating ? '#fbbf24' : '#d1d5db'} />)}
  ...
  return <TouchableOpacity className="bg-white rounded-3xl overflow-hidden border border-[#eb278c2b]" style={{
    ...
    elevation: 3
      ...
      <View className="relative">
        ...
        uri: vendor.image
      }} className="w-full h-36" resizeMode="cover" /> : <LinearGradient colors={['#f3f4f6', '#e5e7eb']} className="w-full h-36 items-center justify-center">
      ...
      {}
      <View className="p-3"> {}
        <Text className="text-sm font-bold text-gray-900 mb-1.5" numberOfLines={2}> {}
          {vendor.businessName}
        </Text>

        {}
        <View className="flex-row items-center mb-2"> {}
          <View className="w-5 h-5 rounded-full bg-pink-100 items-center justify-center mr-1.5"> {}
            <Ionicons name="location" size={10} color="#eb278d" /> {}
          </View>
        ...
        <View className="flex-row items-center justify-between mb-2.5"> {}
          ...
          <View className="bg-pink-50 px-2 py-0.5 rounded-full">
            <Text className="text-pink-600 text-[9px] font-bold"> {}
