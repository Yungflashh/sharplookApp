...
const BookingDetailScreen: React.FC = () => {
  ...
  const renderActionButtons = () => {
    ...
    return (
      <View style={{ gap: 12 }}>
        {/* Complete Payment */}
        {status === 'pending' && booking.paymentStatus !== 'escrowed' && (
          <TouchableOpacity
        ...
        <View className="px-5 py-4" style={{ gap: 16 }}>
          ...
          >
            ...
            <View className="flex-row items-center mb-4">
              <View className="w-16 h-16 rounded-full bg-gradient-to-br from-pink-400 to-pink-600 items-center justify-center mr-3 overflow-hidden">
                {booking.vendor?.avatar ? (
                  <Image
                    source={{ uri: booking.vendor.avatar }}
            ...
            <View className="flex-row" style={{ gap: 12 }}>
              <TouchableOpacity
                onPress={() => handleCall(booking.vendor?.phone)}
              ...
              <TouchableOpacity
                onPress={() => handleMessage(booking.vendor?.phone)}
