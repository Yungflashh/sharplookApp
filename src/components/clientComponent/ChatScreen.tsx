...
const ChatScreen: React.FC = () => {
  ...
  return <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
      ...
      y: 1
        ...
        <View className="px-5 mt-2">
          <View className="flex-row items-center bg-white/20 rounded-2xl px-4 py-3">
            ...
            <TextInput className="flex-1 ml-2 text-sm text-white" placeholder="Search conversations..." placeholderTextColor="rgba(255, 255, 255, 0.7)" value={searchQuery} onChangeText={setSearchQuery} />
        ...
        <View className="flex-row px-5 mt-4 gap-2">
          ...
          <TouchableOpacity onPress={() => setActiveTab('unread')} className={`flex-1 py-2.5 rounded-xl ${activeTab === 'unread' ? 'bg-white' : 'bg-white/20'}`} activeOpacity={0.7}>
            <View className="flex-row items-center justify-center">
              <Text className={`text-center font-semibold text-sm ${activeTab === 'unread' ? 'text-pink-600' : 'text-white'}`}>
                Unread
              </Text>
              {totalUnread > 0 && <View className="ml-2 bg-pink-500 px-2 py-0.5 rounded-full min-w-[20px] items-center">
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
        {filteredConversations.length > 0 ? <View className="py-2">
            {filteredConversations.map(conversation => <TouchableOpacity key={conversation.id} className="bg-white mx-4 mb-2 rounded-2xl overflow-hidden" style={{
          ...Platform.select({
            ios: {
              shadowColor: '#000',
              shadowOffset: {
        ...
        }} activeOpacity={0.7} onPress={() => navigation.navigate('ChatDetail', {
          ...
          vendorName: conversation.vendorName
                ...
                <View className="flex-row items-center p-4">
                  ...
                  <View className="relative mr-3">
                    ...
                    {conversation.isOnline && <View className="absolute bottom-0 right-0 w-4 h-4 bg-green-500 rounded-full border-2 border-white" />}
                  </View>
                  ...
                  <View className="flex-1">
                    ...
                    <View className="flex-row items-center justify-between">
                      <Text className={`text-sm flex-1 pr-2 ${conversation.unreadCount > 0 ? 'text-gray-900 font-semibold' : 'text-gray-500'}`} numberOfLines={1}>
                        {conversation.lastMessage}
                      </Text>
                      {conversation.unreadCount > 0 && <View className="bg-pink-500 rounded-full min-w-[24px] h-6 items-center justify-center px-2">
          ...
          </View> : <View className="flex-1 items-center justify-center px-5 py-20">
            ...
            <Text className="text-sm text-gray-500 text-center mb-6">
              {searchQuery ? 'No results found for your search' : 'Start chatting with vendors to see your conversations here'}
            </Text>
          </View>}
      </ScrollView>

      {}
      <View className="px-5 py-3 bg-white border-t border-gray-200" style={{
      ...Platform.select({
        ios: {
          shadowColor: '#000',
          shadowOffset: {
        ...
        <View className="flex-row gap-2">
          <TouchableOpacity className="flex-1 flex-row items-center justify-center bg-pink-50 py-3 rounded-xl" activeOpacity={0.7}>
            <Ionicons name="search" size={18} color="#eb278d" />
            <Text className="text-pink-600 font-semibold text-sm ml-2">Search Vendors</Text>
          </TouchableOpacity>
          <TouchableOpacity className="flex-1 flex-row items-center justify-center bg-pink-500 py-3 rounded-xl" activeOpacity={0.8} style={{
          shadowColor: '#eb278d',
          shadowOffset: {
            width: 0,
            height: 2
