...
const FilterModal: React.FC<FilterModalProps> = ({ visible, onClose, onApplyFilters }) => {
  ...
  const categories = [
    ...
    { id: 'spa', label: 'Spa & Treatment', icon: 'flower' },
    { id: 'skincare', label: 'Skincare', icon: 'water' },
    { id: 'makeup', label: 'Makeup', icon: 'brush' },
    { id: 'hair', label: 'Hair Styling', icon: 'cut' },
  ];
  ...
  const sortOptions = [
    ...
    { id: 'nearest', label: 'Nearest First', icon: 'location' },
              ...
              {sortOptions.map((option) => (
                <TouchableOpacity
                  ...
                  className={`flex-row items-center px-4 py-3 rounded-xl ${
                    selectedSortBy === option.id ? 'bg-pink-500' : 'bg-white'
                  }`}
                  style={{
                ...
                >
                  <Ionicons
                    ...
                    size={18}
                    color={selectedSortBy === option.id ? '#fff' : '#eb278d'}
                  />
                  <Text
                    className={`ml-2 text-sm font-semibold ${
                      selectedSortBy === option.id ? 'text-white' : 'text-gray-700'
                    }`}
                  >
                    {option.label}
                  </Text>
            ...
            <View className="flex-row flex-wrap gap-2">
              {categories.map((category) => (
                <TouchableOpacity
                  ...
                  className={`flex-row items-center px-4 py-3 rounded-xl ${
                    selectedCategory === category.id ? 'bg-pink-500' : 'bg-white'
                  }`}
                  style={{
              ...
              {priceRanges.map((range, index) => (
                <TouchableOpacity
                  ...
                  className={`flex-row items-center justify-between p-4 ${
                    index !== priceRanges.length - 1 ? 'border-b border-gray-100' : ''
                  }`}
                  activeOpacity={0.7}
                >
                  ...
                  <View
                    className={`w-6 h-6 rounded-full items-center justify-center ${
                      selectedPriceRange === range.id ? 'bg-pink-500' : 'border-2 border-gray-300'
                    }`}
                  >
            ...
            <View className="bg-white rounded-2xl overflow-hidden">
              {ratings.map((rating, index) => (
                ...
                >
                  ...
                  <View
                    className={`w-6 h-6 rounded-full items-center justify-center ${
                      selectedRating === rating.value ? 'bg-pink-500' : 'border-2 border-gray-300'
                    }`}
                  >
            ...
            <View className="bg-white rounded-2xl overflow-hidden">
              {distances.map((distance, index) => (
                <TouchableOpacity
                  ...
                  className={`flex-row items-center justify-between p-4 ${
                    index !== distances.length - 1 ? 'border-b border-gray-100' : ''
                  }`}
                  activeOpacity={0.7}
                >
                  ...
                  <View
                    className={`w-6 h-6 rounded-full items-center justify-center ${
                      selectedDistance === distance.id ? 'bg-pink-500' : 'border-2 border-gray-300'
                    }`}
                  >
          ...
          <View className="px-5 py-5 border-t border-gray-200">
            ...
            <View className="bg-white rounded-2xl overflow-hidden">
              {availability.map((avail, index) => (
                <TouchableOpacity
                  key={avail.id}
                  onPress={() => setSelectedAvailability(avail.id)}
                  className={`flex-row items-center justify-between p-4 ${
