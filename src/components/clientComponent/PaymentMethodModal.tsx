...
import { sharpPayAPI, handleAPIError } from '@/api/api';
...
const PaymentMethodModal: React.FC<PaymentMethodModalProps> = ({
  ...
  const checkWalletBalance = async () => {
    try {
      ...
      const canPay = balance >= bookingAmount;
      setCanPayFromWallet(canPay);
      setShortfall(canPay ? 0 : bookingAmount - balance);
    } catch (error) {
 ...
 const handlePayFromWallet = async () => {
  ...
  setConfirmModal({
    ...
    onConfirm: () => {
      toast.info('Info', 'Wallet payment for existing bookings is not supported. Please use card payment.');
    },
  });
};
