...
const AddServiceModal: React.FC<AddServiceModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const apiError = handleAPIError(error);
      console.error('Error saving service:', apiError);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Info Banner */}
          <View className="bg-pink-50 rounded-xl p-4 mb-5 mt-5 border border-pink-200">
            <View className="flex-row items-center mb-2">
              <Ionicons name="information-circle" size={20} color="#eb278d" />
              <Text className="text-pink-800 font-semibold ml-2">Service Type</Text>
            </View>
            <Text className="text-pink-700 text-sm">
              Your service type (In-Shop, Home Service, or Both) is set in your vendor profile.
              Services will automatically use your profile's vendor type.
            </Text>
          </View>
          ...
          <View className="mb-6">
            ...
            <TextInput
              className={`border ${
                errors.name ? 'border-[#FF0000]' : 'border-gray-300'
              } rounded-xl px-4 py-3.5 text-base text-gray-900 bg-gray-50`}
          ...
          <View className="mb-6">
            ...
            <TextInput
              className={`border ${
                errors.description ? 'border-[#FF0000]' : 'border-gray-300'
              } rounded-xl px-4 py-3.5 text-base text-gray-900 bg-gray-50 h-24`}
            ...
            <View className="flex-row justify-between items-center mt-1">
              {errors.description ? (
                <Text className="text-[#FF0000] text-xs flex-1">{errors.description}</Text>
              ) : (
                <Text className="text-gray-400 text-xs">Min 20 characters</Text>
              )}
          ...
          <View className="mb-6">
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mt-2">
              {categories.map((cat) => (
                <TouchableOpacity
                  ...
                  style={formData.category === cat._id ? { backgroundColor: '#eb278d' } : {}}
                  onPress={() => {
                ...
                >
                  <Text
                    className={`text-sm font-medium ${
                      formData.category === cat._id ? 'text-white' : 'text-gray-700'
                    }`}
                  >
                    {cat.name}
                  </Text>
            ...
            {errors.category && (
              <Text className="text-[#FF0000] text-xs mt-1">{errors.category}</Text>
            )}
          </View>
          ...
          <View className="mb-6">
            ...
            <View className="flex-row items-center gap-3">
              <View
                className={`flex-1 flex-row items-center border ${
                  errors.basePrice ? 'border-[#FF0000]' : 'border-gray-300'
              ...
              <View className="flex-row border border-gray-300 rounded-xl overflow-hidden">
                <TouchableOpacity
                  className={`px-4 py-3 ${formData.priceType === 'fixed' ? '' : 'bg-white'}`}
                  style={formData.priceType === 'fixed' ? { backgroundColor: '#eb278d' } : {}}
                  onPress={() => setFormData({ ...formData, priceType: 'fixed' })}
                >
                  <Text
                    className={`text-sm font-medium ${
                      formData.priceType === 'fixed' ? 'text-white' : 'text-gray-700'
                    }`}
                  >
                    Fixed
                  </Text>
            ...
            {errors.basePrice && (
              <Text className="text-[#FF0000] text-xs mt-1">{errors.basePrice}</Text>
            )}
          ...
          <View className="mb-6">
              ...
              {[15, 30, 45, 60, 90, 120].map((mins) => (
                <TouchableOpacity
                  ...
                  className={`px-4 py-2 rounded-lg ${
                    formData.duration === mins ? '' : 'bg-gray-100 border border-gray-300'
                  }`}
                  style={formData.duration === mins ? { backgroundColor: '#eb278d' } : {}}
                  onPress={() => {
                ...
                >
                  <Text
                    className={`text-sm font-medium ${
                      formData.duration === mins ? 'text-white' : 'text-gray-700'
                    }`}
                  >
                    {mins} min
                  </Text>
            ...
            {errors.duration && (
              <Text className="text-[#FF0000] text-xs mt-1">{errors.duration}</Text>
            )}
          </View>
          ...
          <View className="mb-6">
            ...
            <View className="flex-row items-center gap-3 mb-2">
              <View className="flex-1 flex-row items-center border border-gray-300 rounded-xl px-4 bg-gray-50">
                ...
                <TextInput
                  ...
                  placeholderTextColor="#999"
                  value={(formData.serviceArea.radius / 1000).toString()}
                  onChangeText={(text) => {
            ...
            {formData.serviceArea.coordinates.length > 0 && (
              <View className="flex-row items-center mt-1">
                <Ionicons name="checkmark-circle" size={16} color="#10b981" />
                <Text className="text-xs text-green-600 ml-1">
                  Location set: {formData.serviceArea.coordinates[1].toFixed(4)},{' '}
                  {formData.serviceArea.coordinates[0].toFixed(4)}
                </Text>
              </View>
            )}
            {errors.location && (
              <Text className="text-[#FF0000] text-xs mt-1">{errors.location}</Text>
            )}
            <Text className="text-xs text-gray-500 mt-1">
              Set your location and the radius you're willing to travel for home services
            </Text>
          </View>
          ...
          <View className="mb-8">
            ...
            >
              ...
              <Text className="font-medium mt-2" style={{ color: '#eb278d' }}>
                {selectedImages.length >= 5 ? 'Maximum Images Reached' : 'Upload Images'}
              </Text>
              <Text className="text-xs text-gray-500 mt-1">Max 5 images • JPG, PNG</Text>
            </TouchableOpacity>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mt-3">
                {selectedImages.map((image, index) => (
                  <View key={index} className="relative mr-2">
                    <Image source={{ uri: image.uri }} className="w-20 h-20 rounded-lg" />
                    <TouchableOpacity
                      className="absolute -top-2 -right-2 bg-[#FF0000] rounded-full p-1"
