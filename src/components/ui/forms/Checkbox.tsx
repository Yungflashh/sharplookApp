...
const Checkbox: React.FC<CheckboxProps> = ({
  ...
  const getSizeStyles = () => {
    switch (size) {
      case 'sm':
        return { box: 'w-4 h-4', icon: 12, text: 'text-xs' };
      case 'md':
        return { box: 'w-5 h-5', icon: 14, text: 'text-sm' };
      case 'lg':
        return { box: 'w-6 h-6', icon: 16, text: 'text-base' };
      default:
        return { box: 'w-5 h-5', icon: 14, text: 'text-sm' };
    }
  };
      ...
      >
        ...
        {label && (
          <View className="flex-1">
            {typeof label === 'string' ? (
              <Text className={`${sizeStyles.text} text-gray-700 ${disabled ? 'opacity-50' : ''}`}>
                {label}
              </Text>
            ) : (
              label
