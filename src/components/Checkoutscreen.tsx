...
const CheckoutScreen: React.FC = () => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          ...
          // Backend now deducts wallet and creates the order atomically —
          // no separate payment call needed.
  ...
  const handlePayWithCard = async () => {
    ...
    try {
      setLoading(true);
      // Backend validates the order and initialises Paystack in one step.
      // The order is only created on the backend once payment succeeds.
      const response = await orderAPI.createOrder(buildOrderData('card'));
      if (response.success) {
        const { authorizationUrl, reference, totalAmount } = response.data;
        await cartAPI.clearCart();
        navigation.replace('OrderPayment', { authorizationUrl, reference, totalAmount });
      }
    } catch (error) {
      toast.error('Error', handleAPIError(error).message || 'Failed to initiate payment');
    } finally {
      setLoading(false);
    }
  };
