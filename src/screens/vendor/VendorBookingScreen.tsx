...
import { View, Text, TouchableOpacity, ScrollView, ActivityIndicator, RefreshControl, Alert, TextInput, Platform } from 'react-native';
...
const VendorBookingsScreen: React.FC = () => {
  ...
  const getActionButtons = (booking: VendorBooking) => {
    ...
    if (booking.status === 'pending') {
      return <View className="flex-row" style={{
        gap: 8
      }}>
          <TouchableOpacity onPress={() => handleRejectBooking(booking._id)} disabled={isLoading} className="flex-1 bg-red-500 py-3 rounded-xl" style={{
          shadowColor: '#ef4444',
          shadowOffset: {
            width: 0,
            height: 2
          ...
          elevation: 2
            ...
            {isLoading ? <ActivityIndicator size="small" color="#fff" /> : <Text className="text-white text-center font-bold text-sm">Reject</Text>}
          </TouchableOpacity>

          <TouchableOpacity onPress={() => handleAcceptBooking(booking._id)} disabled={isLoading} className="flex-1 bg-green-500 py-3 rounded-xl" style={{
          shadowColor: '#10b981',
          shadowOffset: {
            width: 0,
            height: 2
          ...
          elevation: 2
            ...
            {isLoading ? <ActivityIndicator size="small" color="#fff" /> : <Text className="text-white text-center font-bold text-sm">Accept</Text>}
          </TouchableOpacity>
        </View>;
    }
    if (booking.status === 'accepted') {
      return <TouchableOpacity onPress={() => handleStartService(booking._id)} disabled={isLoading} className="bg-purple-500 py-3 rounded-xl" style={{
        shadowColor: '#a855f7',
        shadowOffset: {
          width: 0,
          height: 2
        ...
        elevation: 2
          ...
          {isLoading ? <ActivityIndicator size="small" color="#fff" /> : <Text className="text-white text-center font-bold text-sm">Start Service</Text>}
        </TouchableOpacity>;
    }
    if (booking.status === 'in_progress') {
      return <TouchableOpacity onPress={() => handleCompleteService(booking._id)} disabled={isLoading} className="bg-green-500 py-3 rounded-xl" style={{
        shadowColor: '#10b981',
        shadowOffset: {
          width: 0,
          height: 2
        ...
        elevation: 2
          ...
          {isLoading ? <ActivityIndicator size="small" color="#fff" /> : <Text className="text-white text-center font-bold text-sm">Mark Complete</Text>}
        </TouchableOpacity>;
    }
    return null;
  };
  ...
  const renderBookingCard = (booking: VendorBooking) => <TouchableOpacity key={booking._id} onPress={() => navigation.navigate('BookingDetail', {
    bookingId: booking._id
  })} activeOpacity={0.95}>
      <View className="bg-white rounded-3xl p-5 mb-4" style={{
      ...Platform.select({
        ios: {
          shadowColor: '#000',
          shadowOffset: {
        ...
        {}
        <View className="bg-gray-50 rounded-2xl p-4 mb-4" style={{
        gap: 12
      }}>
          <View className="flex-row items-center">
          ...
          <View className="flex-row items-center justify-between">
            ...
            <View className={`px-3 py-1.5 rounded-xl ${booking.paymentStatus === 'escrowed' ? 'bg-blue-100' : booking.paymentStatus === 'released' ? 'bg-green-100' : 'bg-yellow-100'}`}>
              <Text className={`text-xs font-bold ${booking.paymentStatus === 'escrowed' ? 'text-blue-700' : booking.paymentStatus === 'released' ? 'text-green-700' : 'text-yellow-700'}`}>
                {formatStatus(booking.paymentStatus)}
              </Text>
            </View>
          </View>

          {booking.bookingNumber && <View className="flex-row items-center pt-3 border-t border-gray-200">
  ...
  const renderEmptyState = () => <View className="flex-1 items-center justify-center py-20 px-8">
      <LinearGradient colors={['#fce7f3', '#fdf2f8']} className="w-32 h-32 rounded-full items-center justify-center mb-6">
      ...
      <Text className="text-gray-600 text-center text-sm">
        {activeFilter !== 'all' ? `You don't have any ${formatStatus(activeFilter).toLowerCase()} bookings` : "You don't have any bookings yet. They'll appear here once clients book your services."}
      </Text>
    </View>;
  if (loading && page === 1) {
  ...
  <View className="px-5 pt-4">
    <View className="flex-row items-center justify-between mb-4">
      ...
      <View className="flex-row gap-2">
        <TouchableOpacity onPress={() => navigation.navigate('VendorMyResponses')} className="w-10 h-10 rounded-full bg-white/20 items-center justify-center">
          <Ionicons name="chatbox-ellipses" size={20} color="#fff" />
        </TouchableOpacity>
        
        <TouchableOpacity onPress={() => navigation.navigate('AvailableOffers')} className="w-10 h-10 rounded-full bg-white/20 items-center justify-center">
    ...
    <View className="flex-row mb-4" style={{
      ...
      <TouchableOpacity onPress={() => navigation.navigate('AvailableOffers')} className="flex-1 bg-white/20 rounded-xl p-3 flex-row items-center" activeOpacity={0.7}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('VendorMyResponses')} className="flex-1 bg-white/20 rounded-xl p-3 flex-row items-center" activeOpacity={0.7}>
        <View className="w-10 h-10 rounded-full bg-white/30 items-center justify-center mr-3">
    ...
    {}
    {stats && <View className="flex-row mb-4" style={{
          gap: 12
        }}>
        <View className="flex-1 bg-white/20 rounded-2xl p-4">
          ...
          <Text className="text-white/70 text-[10px] mt-1">
            From {stats.completedBookings} completed {stats.completedBookings === 1 ? 'job' : 'jobs'}
          </Text>
        </View>
    ...
    <View className="flex-row items-center bg-white/20 rounded-2xl px-4 py-3">
      ...
      <TextInput className="flex-1 ml-2 text-base text-white" placeholder="Search bookings..." placeholderTextColor="rgba(255, 255, 255, 0.7)" value={searchQuery} onChangeText={setSearchQuery} />
  ...
  {}
  <ScrollView horizontal showsHorizontalScrollIndicator={false} className="my-4" contentContainerStyle={{
        paddingHorizontal: 20,
        gap: 8
      }}>
    {filters.map(filter => <TouchableOpacity key={filter.key} onPress={() => setActiveFilter(filter.key)} className={`px-5 py-2.5 rounded-full ${activeFilter === filter.key ? 'bg-white' : 'bg-white/20'}`} activeOpacity={0.7}>
        <Text className={`font-bold text-sm ${activeFilter === filter.key ? 'text-pink-600' : 'text-white'}`}>
          {filter.label}
      ...
      if (isCloseToBottom) {
        ...
        <View className="px-5 py-4">
          {filteredBookings.length > 0 ? <>
              ...
              {!hasMore && filteredBookings.length > 10 && <Text className="text-center text-gray-400 text-sm py-4">No more bookings</Text>}
            </> : renderEmptyState()}
        </View>
      </ScrollView>
    </SafeAreaView>;
