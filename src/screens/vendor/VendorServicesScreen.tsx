...
import api, { handleAPIError, categoriesAPI } from '@/api/api';
...
const VendorServicesScreen = () => {
  ...
  const handleDeleteService = async () => {
    ...
    setLoading(true);
    try {
      const response = await api.delete(`/services/${selectedService._id}`);
      if (response.data.success) {
  ...
  return <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
      ...
      if (selectedService) {
        handleUpdateService(selectedService._id, serviceData);
      } else {
