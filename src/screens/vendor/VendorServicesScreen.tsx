...
const applyFilters = () => {
  ...
  try {
    // Guard against undefined or non-array services
    ...
    if (filters.searchName.trim()) {
      ...
      filtered = filtered.filter((service) =>
        service.name.toLowerCase().includes(filters.searchName.toLowerCase())
      );
    ...
    <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
      ...
      >
        ...
        >
          ...
          {hasActiveFilters() && (
            <View className="absolute top-0 right-0 w-3 h-3 bg-yellow-400 rounded-full border-2 border-white" />
          )}
        </TouchableOpacity>
      </View>
      ...
      >
        {/* Add Service Button */}
        <TouchableOpacity
          className="flex-row items-center justify-center py-4 mx-5 mt-5 rounded-xl shadow-lg"
          style={{ backgroundColor: '#eb278d' }}
          onPress={() => {
            setSelectedService(null);
            setShowAddModal(true);
        ...
        {loading && services.length === 0 ? (
          ...
          <View className="p-5 pt-4 flex-row flex-wrap gap-3">
