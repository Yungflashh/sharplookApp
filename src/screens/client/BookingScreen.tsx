...
const BookingsScreen: React.FC = () => {
  ...
  const fetchBookings = useCallback(async (isRefresh = false) => {
    try {
      ...
      const res = await bookingAPI.getMyBookings({ role: 'client', page: 1, limit: 50 });
      if (res.success) {
        const data: Booking[] = Array.isArray(res.data) ? res.data : res.data.bookings || [];
        setBookings(data);
      }
    } catch (e) {
        ...
        {nextBooking && countdown && (
          ...
          >
            <View style={{ flex: 1, marginRight: 12 }}>
              ...
              <Text style={ss.bannerName} numberOfLines={1}>{getVendorName(nextBooking)}</Text>
            </View>
          ...
          <View style={ss.empty}>
            ...
            {activeTab === 'all' && (
              <TouchableOpacity
                ...
                activeOpacity={0.85}
              >
                <LinearGradient colors={[PRIMARY, PRI_DARK]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={ss.exploreBtnInner}>
                  <Ionicons name="search-outline" size={16} color="#fff" />
                  <Text style={ss.exploreBtnTxt}>Explore Services</Text>
                </LinearGradient>
              </TouchableOpacity>
            )}
          </View>
          ...
          filtered.map((booking) => {
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.85}
              >
                <AvatarCircle uri={booking.vendor?.avatar} name={vendorName} />
                <View style={ss.cardMid}>
                  <Text style={ss.cardName} numberOfLines={1}>{vendorName}</Text>
