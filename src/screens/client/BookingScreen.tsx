...
const BookingsScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(booking => booking.service?.name?.toLowerCase().includes(searchQuery.toLowerCase()) || booking.vendor?.vendorProfile?.businessName?.toLowerCase().includes(searchQuery.toLowerCase()) || `${booking.vendor?.firstName || ''} ${booking.vendor?.lastName || ''}`.toLowerCase().includes(searchQuery.toLowerCase()) || booking.bookingNumber?.toLowerCase().includes(searchQuery.toLowerCase()));
    }
  ...
  const renderBookingCard = (booking: Booking) => <TouchableOpacity key={booking._id} className="bg-white rounded-2xl p-4 mb-3 shadow-sm" style={{
    shadowColor: '#000',
    shadowOffset: {
      width: 0,
      height: 1
  ...
  const renderEmptyState = () => <View className="flex-1 items-center justify-center py-20">
      ...
      <Text className="text-gray-600 text-center mb-6 px-8">
        {activeFilter !== 'all' ? `You don't have any ${activeFilter} bookings` : "You haven't made any bookings yet. Start by exploring services!"}
      </Text>
      {activeFilter === 'all' && <TouchableOpacity className="bg-pink-600 px-6 py-3 rounded-xl" onPress={() => navigation.navigate('AllVendors')}>
