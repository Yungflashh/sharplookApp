...
const ClientProfileScreen: React.FC = () => {
  ...
  const renderIcon = (iconFamily: string = 'ionicons', iconName: string, size: number = 22, color: string = '#eb278d') => {
  ...
  return <SafeAreaView className="flex-1 bg-gray-50" edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
        ...
        <View className="pb-6 rounded-b-[50px] bg-[#eb278d]">
          ...
          <View className="items-center pb-5 px-5">
            <TouchableOpacity activeOpacity={0.8} className="relative mb-4">
              ...
              <TouchableOpacity className="absolute bottom-0 right-0 w-9 h-9 rounded-full bg-pink-500 items-center justify-center border-3 border-white" activeOpacity={0.7} style={{
              shadowColor: '#eb278d',
              shadowOffset: {
                width: 0,
                height: 2
        ...
        <View className="pt-5 pb-20">
          {menuSections.map((section, sectionIndex) => <View key={sectionIndex} className="mb-5 px-5">
            ...
            ...Platform.select({
              ...
              android: {
                ...
                {section.items.map((item, itemIndex) => <TouchableOpacity key={itemIndex} className={`flex-row items-center p-4 ${itemIndex !== section.items.length - 1 ? 'border-b border-gray-100' : ''}`} onPress={item.onPress} activeOpacity={0.7}>
          ...
          <View className="mb-5 px-5">
            ...
            ...Platform.select({
              ...
              <TouchableOpacity className="flex-row items-center p-4 bg-red-50 border-b border-gray-100" onPress={() => setShowLogoutModal(true)} activeOpacity={0.7}>
              ...
              <TouchableOpacity className="flex-row items-center p-4 bg-red-50" onPress={() => setShowDeleteModal(true)} activeOpacity={0.7}>
                <View className="w-11 h-11 rounded-xl bg-red-100 items-center justify-center mr-3">
      ...
      {}
      <ConfirmationModal visible={showLogoutModal} title="Logout" message="Are you sure you want to logout?" icon="log-out-outline" iconColor="#eb278d" confirmText="Yes, Logout" cancelText="Cancel" confirmColor="#eb278d" loading={loading} onConfirm={handleLogout} onCancel={() => setShowLogoutModal(false)} />

      <ConfirmationModal visible={showDeleteModal} title="Delete Account" message="Are you sure you want to delete your account? This action cannot be undone." icon="warning-outline" iconColor="#ef4444" confirmText="Delete Account" cancelText="Cancel" confirmColor="#ef4444" loading={loading} onConfirm={handleDeleteAccount} onCancel={() => setShowDeleteModal(false)} />
    </SafeAreaView>;
};
